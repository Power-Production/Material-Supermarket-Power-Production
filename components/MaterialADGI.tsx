...
export function MaterialADGI({ history, onUpdateADGIStatus }: MaterialADGIProps) {
  ...
  const filteredHistory = consolidatedMaterials
    .filter(material => {
      ...
      // Date filter - check if any entry from this material matches the selected date
      const matchesDate = !selectedDate || 
        material.entries.some(entry => 
          entry.timestamp.toDateString() === selectedDate.toDateString()
        );
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-gradient-to-r from-gray-50 to-orange-50">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Filter & Search ADGI</h3>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
            <PopoverTrigger asChild>
              <Button 
                variant="outline" 
                className={`w-full md:w-48 justify-start text-left font-normal bg-white ${!selectedDate && "text-muted-foreground"}`}
              >
